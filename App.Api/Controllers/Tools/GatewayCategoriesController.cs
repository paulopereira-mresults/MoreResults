...
namespace App.Api.Controllers.Tools
{
  ...
  {
    public GatewayCategoriesController(IUnitOfWork uow) : base(uow)
    {
    }
    ...
    [HttpPost]
    public async Task<IActionResult> Add([FromBody] GatewayCategory category, CancellationToken cancellationToken)
    {
      DefaultResponseDto<GatewayCategory> response = await UnitOfWork
          .Rules
          .GatewayCategory
          .Add(category, cancellationToken);
